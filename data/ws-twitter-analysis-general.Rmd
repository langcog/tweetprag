...
```{r}
fdf2$numrunners <- with(fdf2,as.numeric(!is.na(b1))+as.numeric(!is.na(b2))+as.numeric(!is.na(b3)))
fdf2$tent <- fdf2$ent*fdf2$nwords
#AB-by-AB analysis for LI, etc.
fms <- ddply(fdf2, .(atbatnum,gamenum), summarise,
      len = mean(tsize),
      ent = mean(entropy),
      tent = mean(tent),
...
```{r}
#Minute-by-minute analysis for the cases where at-bats don;t matter
fmsh <- ddply(fdf2, .(h,gamenum), summarise,
      len = mean(tsize),
      ent = mean(entropy),
      tent = mean(tent),
      he = mean(he),
...
qplot(starttime,ent,data=fmsh,color=as.factor(gamenum)) + geom_smooth(aes(group=1),size=2) + labs(title='Time vs. per-word entropy\nWorld Series, by minute.',x='Time (hrs)',y='Per-word entropy',color='Game #')
```

```{r}
#Rate - Per-word entropy - Agg
qplot(log10(rate),ent,data=fms,color=starttime) + geom_smooth(method='lm') + labs(title='Tweet rate vs. per-word entropy\nWorld Series, by game, AB-by-AB.',x='Log_10 tweet rate',y='Per-word entropy',color='Time\n(hrs UTC)')

#Rate - Total entropy - Agg
qplot(log10(rate),tent,data=fms,color=starttime) + geom_smooth(method='lm') + labs(title='Tweet rate vs. total tweet entropy\nWorld Series, by game, AB-by-AB.',x='Log_10 tweet rate',y='Total tweet entropy',color='Time\n(hrs UTC)')

#Rate - Length - Agg
qplot(log10(rate),len,data=fms,color=starttime) + geom_smooth(method='lm') + labs(title='Tweet rate vs. tweet length\nWorld Series, by game, AB-by-AB.',x='Log_10 tweet rate',y='Tweet length',color='Time\n(hrs UTC)')

#Time - Per-word entropy - Agg
qplot(starttime,ent,data=fms,color=as.factor(gamenum)) + geom_smooth(aes(group=1)) + labs(title='Time vs. per-word entropy\nWorld Series, by game, AB-by-AB.',x='Time (hrs UTC)',y='Per-word entropy',color='Game #')

#Time - Per-word entropy - Split
qplot(starttime,ent,data=fms,color='blue') + facet_wrap(~gamenum) + geom_smooth() + labs(title='Time vs. per-word entropy\nWorld Series, by game, AB-by-AB.',x='Time (hrs UTC)',y='Per-word entropy')


```

```{r}
qplot(as.factor(abs(scorediff)),log10(rate),data=fms) + geom_violin()
```
