...
We begin by describing our corpus and our method of calculating linguistic content (by computing entropy within a simple $n$-gram model). We then investigate both gradual changes in information content as the event goes on (testing adaptation driven by contextual mutual information in Equation \ref{eq:joint-entropy}, replicating \citealt{genzel2002}) and rapid changes in information content in response to important in-game events (testing adaptation driven by non-linguistic information in Equation \ref{eq:joint-entropy}). We end by considering a number of control analyses that provide evidence against alternative accounts of our results. 
...
Figure \ref{fig:time-perword-ent} shows evidence for changes in per-word entropy over the course of games. Per-word entropy rises sharply in the first half-hour of each game, then begins to level off and finally declines slightly over time.  This pattern is consistent with the constant entropy rate proposal of \cite{genzel2002}, and more specifically with the context decay model of \citealt{qian2012}.\footnote{A late decline in per-word entropy also occasionally appeared in \citealt{qian2012}'s data.} 

We used mixed-effects linear regression to quantify this relationship, using the time of an at-bat to predict both per-word and per-tweet entropy during the at-bat. Specifically, we used the logarithm of time as our fixed-effect predictor, per \citealt{qian2012}.  We added game-specific random intercepts and slopes of log-time to capture cross-game variation. This model showed significant positive effects of time on entropy, using likelihood-ratio tests for both models (per-word entropy: $.348 \pm .045; p<.001, \chi^2(3)=104.6$, per-tweet entropy: $10.31 \pm 2.08; p=.001, \chi^2(3)=74.65$). %Significant positive effects of time on entropy are found through likelihood-ratio tests for both models (per-word: $.080 \pm .016; p<.001, \chi^2(1)=11.02$, per-tweet: $2.61 \pm .74; p=.008, \chi^2(1)=7.12$).
...
% \footnote{Blowouts are games whose outcome is easy to predict and hence uninteresting.}
...
Our work highlights the importance of non-linguistic common ground for the informational content of language. This relationship is widely acknowledged in theories of pragmatic communication \cite{grice1975,sperber1986,clark1996,frank2012}, but has been largely absent in information-theoretic treatments of linguistic complexity. The omission of this information has largely been for practical, rather than theoretical, reasons: As \citealt{genzel2002} note, it is typically very difficult to compute semantic---let alone non-linguistic---information content. Due to the abundance of available metadata, internet communications surrounding shared media events may be a promising source of grounded language use where context can be quantified more effectively. 

A substantial recent literature suggests that information content is the critical variable governing processing difficulty in language comprehension \cite{levy2008,demberg2008,boston2008,smith2013}. Under surprisal theory \cite{hale2001,levy2008}, the overall predictability of individual elements of language is due to a predictive model of their likelihood in the current context. Given this model of processing difficulty, our work here makes a strong prediction: that the information processing difficulty of a word or sentence should track with its total information content (including its relationship to the non-linguistic context), rather than its linguistic information content alone. Some preliminary evidence supports this idea. In a study of the processing complexity of negative utterances, \citealt{nordmeyer2014} found that the processing cost of negation was predicted by the surprisal of the negation in a particular pragmatic context. Future work should test this hypothesis across a wider variety of structures and contexts. 
